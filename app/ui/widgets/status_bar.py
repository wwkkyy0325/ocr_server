...
class DynamicStatusBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        try:
            ...
            logger.error("status_bar", "tick_register_failed", f"Failed to register StatusBarUpdater to TickScheduler: {e}")

        # 注册到日志控制器
        try:
            from app.log.log_bus import get_logger, LogLevel
            logger_instance = get_logger()
            logger_instance.set_status_callback(self.set_status)
            logger_instance.enable_component("ocr_subprocess")
            logger_instance.enable_component("processing")
            logger_instance.enable_component("ui")
        except Exception as e:
            logger.error("status_bar", "logger_register_failed", f"Failed to register with LoggerController: {e}")
    ...
    def set_status(self, text, status_type=STATUS_INFO):
        ...
        Args:
            text: 显示文本
            status_type: 状态类型 (ready, info, success, warning, error, working) 或 LogLevel
        """
        self.base_text = text
        self.label.setText(text)

        # 将 LogLevel 映射为状态栏常量
        from app.log.log_bus import LogLevel
    ...
    def _update_system_status(self):
        ...
        try:
            ...
            # 但如果处于初始化锁定状态，不要覆盖提示
            if not self.is_animating and not self.initializing_lock and (
                    self.base_text in ["就绪", ""] or self.base_text.startswith("系统状态:")):
                # 构建综合状态信息
                status_parts = []
                if ocr_status:
                ...
                if status_parts:
                    ...
                    if self.label.text() != status_text:
                        ...
                        self.indicator.setStyleSheet(
                            f"background-color: {self.colors[self.STATUS_INFO]}; border-radius: 5px;")
