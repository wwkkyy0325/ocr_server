...
class ResultTableWidget(QWidget):
    ...
    def auto_fit_all(self, resize_splitter=True, remember=True):
        """
        自动调整所有列宽和行高以适应内容
        
        Args:
            resize_splitter: 是否请求调整分割器宽度
            remember: 是否记住当前的尺寸设置
        """
        self._auto_fit_all(resize_splitter=resize_splitter, remember=remember)
