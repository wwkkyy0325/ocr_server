...
class UIConstraintManager(QObject):
    ...
    - Action (动作): 对某个 UI 组件的具体操作（如 setEnabled, setChecked, setVisible）。
    """
    ...
    def _evaluate_rules(self, key=None, value=None):
        """评估所有规则"""
        # 获取当前配置快照
        ...
        for rule in self._rules:
            try:
                ...
                print(f"Error evaluating UI constraint rule: {e}")
        ...
        def mutex_rule(config):
            ...
            if is_triggered and target_keys:
                for t_key in target_keys:
                    ...
                    if config.get(t_key):
                        print(f"[UIConstraint] Mutex triggered: {trigger_key}={trigger_value} -> Force disabling {t_key}")
