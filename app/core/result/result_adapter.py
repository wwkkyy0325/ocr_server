...
标准前端数据项结构：
{
    ...
    'source': str,              # 数据来源标记 ('ocr', 'table_ai')
...
class ResultAdapter:
    ...
    @classmethod
    @handle_errors(error_code=ErrorCode.RESULT_FORMAT_001, fallback_return=[], component="ResultAdapter")
    def adapt(cls, raw_data: Any, source_type: str = "auto") -> List[Dict[str, Any]]:
        ...
        try:
            # DEBUG 日志：适配开始
            logger.debug("result_adapter", "adapt_start",
                         f"开始适配 - 类型：{type(raw_data).__name__}")

            if isinstance(raw_data, (dict, list)) and len(raw_data) > 0:
                if isinstance(raw_data, dict):
                    logger.debug("result_adapter", "adapt_debug",
                                 f"字典键：{list(raw_data.keys())[:10]}")
                if isinstance(raw_data, list):
                    logger.debug("result_adapter", "adapt_debug",
                                 f"列表长度：{len(raw_data)}")
                    if raw_data and isinstance(raw_data[0], dict):
                        logger.debug("result_adapter", "adapt_debug",
                                     f"首项键：{list(raw_data[0].keys())[:10]}")
            ...
            logger.debug("result_adapter", "adapt_detected",
                         f"检测格式：{detected_format}, 区域数：{len(regions) if regions else 0}")
            ...
            for idx, region in enumerate(regions):
                try:
                    ...
                    logger.warning("result_adapter", "adapt_region_failed",
                                   f"适配区域 {idx} 失败：{e}")
                    continue

            logger.info("result_adapter", "adapt_complete",
                        f"适配完成 - 输出 {len(adapted_items)} 项")

            if adapted_items:
                logger.debug("result_adapter", "adapt_sample",
                             f"首项键：{list(adapted_items[0].keys())}, "
                             f"有 box: {'box' in adapted_items[0]}, "
                             f"有 polygon: {'polygon' in adapted_items[0]}")

            return adapted_items

        except Exception as e:
            logger.error("result_adapter", "adapt_error", f"适配过程出错：{e}")
            import traceback
            traceback.print_exc()
            return []

    @classmethod
    @handle_errors(error_code=ErrorCode.RESULT_FORMAT_001, fallback_return=('ocr_list', []), component="ResultAdapter")
    def _detect_and_extract(cls, raw_data: Any) -> Tuple[str, Optional[List]]:
        ...
        # Case 3: 其他格式
            ...
            logger = get_logger()
            logger.warning("result_adapter", "unknown_format",
                           f"未知数据格式：{type(raw_data)}")
            return cls.FORMAT_OCR_LIST, []

    @classmethod
    @handle_errors(error_code=ErrorCode.RESULT_FORMAT_001, fallback_return=None, component="ResultAdapter")
    ...
    @staticmethod
    @handle_errors(error_code=ErrorCode.RESULT_FORMAT_001, fallback_return=0.0, component="ResultAdapter")
    ...
    @classmethod
    @handle_errors(error_code=ErrorCode.RESULT_FORMAT_001, fallback_return=([0, 0, 0, 0], []), component="ResultAdapter")
    ...
    def merge_results(results: List[List[Dict]]) -> List[Dict]:
        """
        合并多个 OCR 结果（已废弃，请使用 ResultProcessor.merge_results）
        
        Deprecated:
            使用 ResultProcessor.merge_results() 替代
        """
        from app.core.result.result_processor import ResultProcessor
        return ResultProcessor.merge_results(results)

    @staticmethod
    @handle_errors(error_code=ErrorCode.RESULT_FORMAT_001, fallback_return=[], component="ResultAdapter")
    def filter_empty(items: List[Dict]) -> List[Dict]:
        """
        过滤空文本项（已废弃，请使用 ResultProcessor.filter_empty）
        
        Deprecated:
            使用 ResultProcessor.filter_empty() 替代
        """
        from app.core.result.result_processor import ResultProcessor
        return ResultProcessor.filter_empty(items)

    @staticmethod
    @handle_errors(error_code=ErrorCode.RESULT_FORMAT_001, fallback_return=[], component="ResultAdapter")
    def sort_by_position(items: List[Dict], reading_order: str = 'lr-tb') -> List[Dict]:
        """
        按位置排序结果（已废弃，请使用 ResultProcessor.sort_by_position）
        
        Deprecated:
            使用 ResultProcessor.sort_by_position() 替代
        """
        from app.core.result.result_processor import ResultProcessor
        return ResultProcessor.sort_by_position(items, reading_order)
