# Path: src/app/core/process_manager.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
class ProcessManager:
    ...
    def _create_queues(self):
        ...
        """
        # 输入队列（文件路径）
        self.queues['input'] = multiprocessing.Queue()
        
        # 处理队列（图像数据和元数据）
        self.queues['processing'] = multiprocessing.Queue()
        
        # 输出队列（处理结果）
        self.queues['output'] = multiprocessing.Queue()
    ...
    def stop_processes(self):
        ...
        try:
            for _ in range(len(self.processes) + 5):  # 发送足够多的停止信号
                if not self.queues['control'].closed:
    ...
    def _input_worker(self):
        ...
        while self.running:
            try:
                # 检查是否有处理任务
                try:
                    if not self.queues['input'].closed:
                        ...
                        if task.get('type') == 'STOP':
                            ...
                            for image_path in image_files:
                                ...
                                if not self.queues['processing'].closed:
            ...
            try:
                ...
                try:
                    if not self.queues['processing'].closed:
                        ...
                        # 添加到输出队列
                        if self.running and not self.queues['output'].closed:
    ...
    def _output_worker(self):
        ...
        while self.running:
            try:
                ...
                # 检查输出任务
                try:
                    if not self.queues['output'].closed:
